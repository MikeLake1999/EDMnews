...
<body data-rsssl="1" class="home page-template page-template-page-15zine-builder page-template-page-15zine-builder-php page page-id-21  cb-sidebar-right  cb-sticky-mm cb-sticky-sb-on  cb-tm-dark cb-body-light cb-menu-light cb-mm-dark cb-footer-dark   cb-m-logo-off cb-m-sticky cb-sw-tm-box cb-sw-header-box cb-sw-menu-box cb-sw-footer-box cb-menu-al-left  cb-fis-b-standard">
    <div class="container">
        ...
        <?php 
        $catalog = $_GET['Name%id'];
 
        // PHẦN XỬ LÝ PHP
        // BƯỚC 1: KẾT NỐI CSDL
        require("mysqli_connect.php");
        // BƯỚC 2: TÌM TỔNG SỐ RECORDS
        $result = mysqli_query($conn, "select count(PostID) as total from posts where CatalogName='".$catalog."'  ");
        ...
        $result = mysqli_query($conn, "SELECT *, DATE_FORMAT(DatePost,'%d/%m/%Y') AS regdate FROM posts WHERE catalogname='".$catalog."' AND ActivePost = 1 ORDER BY PostID DESC LIMIT $start, $limit");
        ?>
        <div class="row top">
            <?php
                     $catalog = $_GET['Name%id'];?>
            <div class="col-lg-9 col-md-9 col-sm-12 content">
            <div class="author-span"><a class="title text-dark"><dt>Home&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;&nbsp;<?php echo $catalog ?></dt></a> </div>
                <div class="content-post">
                    <div class="cb-module-header cb-category-header">
            <h1 class="cb-module-title"><?php echo $catalog ?></h1>                    </div>
                    <?php        
                    while ($row = $result->fetch_array()) {
                        $id=$row['PostID'];
                          
                        echo '<div class="row post ">';
                        echo '<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12"><a href="post.php?id='.$id.'"><img src='.$row['Img'].' class="img-fluid img-contents" alt=""></a></div>';
           ...
           <?php 
            ...
            if ($current_page > 1 && $total_page > 1){
                echo '<button class="btn btn-dark" type="button" ><a style="color:white" href="Catalog.php?Name%id='.$catalog.'&page='.($current_page-1).'">Prev</a></button>';
            }
            ...
            for ($i = 1; $i <= $total_page; $i++){
                ...
                if ($i == $current_page){
                    ...
                    echo '<button type="button" class="btn btn-dark"><a href="Catalog.php?Name%id='.$catalog.'&page='.$i.'" style="color:white">'.$i.'</a></button>';
                }
            }
 
            // nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
            if ($current_page < $total_page && $total_page > 1){
                echo '<button type="button" value="Next" class="btn btn-dark"><a style="color:white" href="Catalog.php?Name%id='.$catalog.'&page='.($current_page+1).'">Next</a></button>';
            }
           ?>
        </div>
